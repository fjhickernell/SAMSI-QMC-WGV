%%Plot results
function sim_eval_iter_plot_results(func_str,ac_flg)

load(['sim_eval_results_' func_str '_ac' int2str(ac_flg) '.mat'], ...
   'rat_vec', 'eps_vec', 'n_vec', 'w_est', 'func', 'd', 'cur_n', ...
   'basisFun', 'gam_mtx', 's_max', 'four_coef_est', 'gam_idx', ...
...
print('-depsc',['sim_eval_results_' func_str '_ac' int2str(ac_flg) 'Err.eps'])
...
[f_app_Vis] = ...
       eval_f_four(x_Vis,basisFun,gam_mtx(gam_idx(1:cur_n),:), ...
       s_max,four_coef_est(1:cur_n));
...
print('-depsc',['sim_eval_results_' func_str '_ac' int2str(ac_flg) 'Fun.eps'])
...
zlabel(['\(f_{\mbox{app}}(x_{' int2str(whCoordBig(1)) '}, x_{' int2str(whCoordBig(2)) ...
   '}, ' num2str(nom_Val) ', \ldots)\)'])
print('-depsc',['sim_eval_results_' func_str '_ac' int2str(ac_flg) 'Appx.eps'])
...
zlabel(['\(\mbox{err}(x_{' int2str(whCoordBig(1)) '}, x_{' int2str(whCoordBig(2)) ...
   '}, ' num2str(nom_Val) ', \ldots)\)'])
print('-depsc',['sim_eval_results_' func_str '_ac' int2str(ac_flg) 'FunErr.eps'])
